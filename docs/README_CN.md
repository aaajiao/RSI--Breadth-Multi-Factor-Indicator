...
- 智能警报只按当前 tick 仍然可见的触发等级发布，不会在可见触发已经消失后再根据旧的 intrabar 最高等级补发
...
- 在手动 `SPY / QQQ / IWM` 显示模式下，`AGG` 共振不会再单独走一条当前图上不可见的隐藏提醒路径；纯共振提醒只属于 `AGG(共振)` 显示路径
- 在 `日内 + Live Alert Data` 模式下，同侧提醒会在整个常规交易时段内锁存，因此 `Lv1` 不会在当天盘中反复提醒
- Live 日内的同侧锁存只会在下一次常规交易时段开盘时重置；在当前时段内，同级重复和降级都会保持静默
